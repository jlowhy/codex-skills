...
When the cleanup removes a function, helper, API, or compatibility path, treat contract-completion cleanup as in scope: follow reachable call sites and any wrappers, tests, mocks, docs, stories, types, or configuration that still depend on that symbol or existed only to support it.
...
- files or boundary being cleaned up
- why any non-touched code is in scope, if cleanup expands beyond the initially changed surface
- what invariant, policy, or architectural boundary each candidate "wrapper" may still own before deleting or inlining it
- structural improvement being attempted
- smallest validation that should prove behavior stayed stable

Apply the smallest high-value pass first:
...
3. remove wrappers, mirror types, and forwarding layers that do not enforce a real invariant, remove repeated wiring, create a stable boundary, or perform precise normalization
4. simplify control flow, data flow, and state transitions
5. extract helpers or subviews only when ownership becomes clearer
